...
class Blockchain {
    ...
    checkBlock(newBlock, previousBlock, referenceBlockchain = this.blocks) {
        ...
        if (previousBlock.index + 1 !== newBlock.index) { // Check if the block is the last one
            ...
            throw new BlockAssertionError(`Invalid proof-of-work difficulty: expected '${newBlock.getDifficulty()}' be smaller than '${this.getDifficulty(newBlock.index)}'`);
        } else if(newBlock.merkleRoot !== Block.generateMerkleRoot(txHashes, newBlock.transactions.length)) {
            console.error(`Invalid merkle root: expected '${newBlock.merkleRoot}' got '${Block.generateMerkleRoot(txHashes, newBlock.transactions.length)}'`);
            throw new BlockAssertionError(`Invalid merkle root: expected '${newBlock.merkleRoot}' got '${Block.generateMerkleRoot(txHashes, newBlock.transactions.length)}'`);    
        }
