...
class Blockchain {
    ...
    getTransactionsByAddress(address) {
        ...
        this.blocks.map(block => {
            const searchTxForInputsOrOutputs = tx => {
                const hasInputs = tx.data.inputs.filter(
                    input => input.address === address
                ).length;
                const hasOutputs = tx.data.outputs.filter(
            ...
            transactions.push(
                block.transactions.filter(searchTxForInputsOrOutputs)
            );
        });

        return transactions.reduce((arr, tx) => arr.concat(tx), []);
    }
    ...
    getTransactionFromBlocks(transactionId) {
        return R.find(
            R.compose(
                R.find(R.propEq('id', transactionId)),
                R.prop('transactions')
            ),
            this.blocks
        );
    ...
    replaceChain(newBlockchain) {
        ...
        let newBlocks = R.takeLast(
            newBlockchain.length - this.blocks.length,
            newBlockchain
        );
    ...
    checkChain(blockchainToValidate) {
        ...
        if (
            ...
            throw new BlockchainAssertionError(
                'Genesis blocks aren\'t the same'
            );
        }
    ...
    addTransaction(newTransaction, emit = true) {
        ...
        if (this.checkTransaction(newTransaction, this.blocks)) {
            ...
            console.debug(
                `Transaction added: ${JSON.stringify(newTransaction)}`
            );
    ...
    checkBlock(newBlock, previousBlock, referenceBlockchain = this.blocks) {
        ...
        const txHashes = R.map(tx => tx.hash, newBlock.transactions);
        const previousIndex = parseInt(previousBlock.index)

        if (previousIndex + 1 != newBlock.index) {
            // Check if the block is the last one
            console.error(
                `Invalid index: expected '${previousIndex + 1}' got '${
                    newBlock.index
                }'`
            );
            ...
            console.error(
                `Invalid previoushash: expected '${previousBlock.hash}' got '${
            ...
            newBlock.getDifficulty() >= this.getDifficulty(newBlock.index)
        ) {
            // If the difficulty level of the proof-of-work challenge is correct
            console.error(
                `Invalid proof-of-work difficulty: expected '${newBlock.getDifficulty()}' to be smaller than '${this.getDifficulty(
                    newBlock.index
                )}'`
            );
            throw new BlockAssertionError(
                `Invalid proof-of-work difficulty: expected '${newBlock.getDifficulty()}' be smaller than '${this.getDifficulty(
                    newBlock.index
                )}'`
            );
        } else if (
            newBlock.merkleRoot !==
            Block.generateMerkleRoot(txHashes, newBlock.transactions.length)
        ) {
            console.error(JSON.stringify(txHashes, null, 3))
            console.error(
                `Invalid merkle root: expected '${
                    newBlock.merkleRoot
                }' got '${Block.generateMerkleRoot(
                    txHashes,
                    newBlock.transactions.length
                )}'`
            );
            throw new BlockAssertionError(
                `Invalid merkle root: expected '${
        ...
        if (!isInputsAmountGreaterOrEqualThanOutputsAmount) {
            console.error(
                `Invalid block balance: inputs sum '${sumOfInputsAmount}', outputs sum '${sumOfOutputsAmount}'`
            );
            throw new BlockAssertionError(
                `Invalid block balance: inputs sum '${sumOfInputsAmount}', outputs sum '${sumOfOutputsAmount}'`,
        ...
        let listOfTransactionIndexInputs = R.flatten(
            R.map(
                R.compose(
                    R.map(
                        R.compose(
                            R.join('|'),
                            R.props(['transaction', 'index'])
                        )
                    ),
                    R.prop('inputs'),
                    R.prop('data')
        ...
        let doubleSpendingList = R.filter(
            x => x >= 2,
            R.map(R.length, R.groupBy(x => x)(listOfTransactionIndexInputs))
        );

        if (R.keys(doubleSpendingList).length) {
            console.error(
                `There are unspent output transactions being used more than once: unspent output transaction: '${R.keys(
                    doubleSpendingList
                ).join(', ')}'`
            );
            throw new BlockAssertionError(
        ...
        let transactionsByType = R.countBy(
            R.prop('type'),
            newBlock.transactions
        );
        if (transactionsByType.fee && transactionsByType.fee > 1) {
            console.error(
                `Invalid fee transaction count: expected '1' got '${
                    transactionsByType.fee
                }'`
            );
            throw new BlockAssertionError(
                `Invalid fee transaction count: expected '1' got '${
        ...
        if (transactionsByType.reward && transactionsByType.reward > 1) {
            console.error(
                `Invalid reward transaction count: expected '1' got '${
                    transactionsByType.reward
                }'`
            );
            throw new BlockAssertionError(
                `Invalid reward transaction count: expected '1' got '${
    ...
    checkTransaction(transaction, referenceBlockchain = this.blocks) {
        ...
        let isInputTransactionsUnspent = R.all(
            R.equals(false),
            R.flatten(
                R.map(txInput => {
                    return R.map(
        ...
        if (!isInputTransactionsUnspent) {
            console.error(
                `Not all inputs are unspent for transaction '${transaction.id}'`
            );
            throw new TransactionAssertionError(
                `Not all inputs are unspent for transaction '${
                    transaction.id
                }'`,
                transaction.data.inputs
            );
    ...
    getUnspentTransactionsForAddress(address) {
        ...
        R.forEach(txOutput => {
            if (
                !R.any(
                    txInput =>
                        txInput.transaction == txOutput.transaction &&
                        txInput.index == txOutput.index,
                    txInputs
                )
            ) {
