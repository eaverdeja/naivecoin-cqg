...
class Miner {
    ...
    mine(rewardAddress, feeAddress) {
        ...
        const thread = spawn(function (input, done) {
            ...
            if(input.port == 3001) {
                ...
                console.assert(`port[${input.port}] Sleeping for ${time} seconds after trying to prove my work...`)
            ...
            console.assert(`Miner on port[${input.port}] found block with hash: ${foundBlock.hash}\nBlock has following tx ids: ${JSON.stringify(txs, null, 3)}`)
    ...
    static generateNextBlock(rewardAddress, feeAddress, blockchain) {
        ...
        let merkleRoot = Block.generateMerkleRoot(txHashes, transactions.length);
    ...
    static proveWorkFor(jsonBlock, difficulty, port) {
        ...
        let start = moment()
        console.assert(`Miner at port[${port}] starting to look for block at: ${start.format('D/M/Y H:m:s.SSS')}`)
        ...
        console.assert(`Miner at port[${port}] found block: difficulty '${difficulty}' hash '${block.hash}' nonce '${block.nonce}'`);
